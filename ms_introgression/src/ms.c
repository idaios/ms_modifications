...
struct params pars ;	
double *samplingTimes, *samplingTimesPop;
double samplingTimesSum;

    int
main(int argc, char *argv[])
{
  int i, k, howmany, segsites, j; 
	char **list, **cmatrix(), **tbsparamstrs ;
	FILE *pf, *fopen() ;
	double probss, tmrca, ttot ;
	void seedit( const char * ) ;
	void getpars( int argc, char *argv[], int *howmany )  ;
	int gensam( char **list, double *probss, double *ptmrca, double *pttot, double *coalTime ) ;
	double comparePops(char** list, int ns, int i1, int i2, int j1, int j2);
	double indComparison(char** list, int ns, int ix, int i1, int i2);
	samplingTimesSum = 0;
	...
	samplingTimesPop = calloc(MAXPOP, sizeof(double));
	getpars( argc, argv, &howmany) ;   /* results are stored in global variable, pars */

	assert(pars.cp.npop < MAXPOP);

	samplingTimes = calloc(pars.cp.nsam, sizeof(double));
	...
	FILE *coalTimeOut = fopen("coalout.txt", "w");
	FILE *compOut = fopen("compout.txt", "w");
	FILE *withinCompOut = fopen("withinComp.txt", "w");
	FILE *betweenPopOutput;

	int jj  = 0;
	for(i = 0; i < pars.cp.npop; ++i)
	  for(j = 0; j < pars.cp.config[i]; ++j)
	    {
	      //fprintf(stderr, "samplingTimesPop %d is : %f\n", i, samplingTimesPop[i]);
	      samplingTimes[jj] = samplingTimesPop[i];
	      samplingTimesSum += samplingTimes[jj];
	      jj++;
	    }
    ...
    while( howmany-count++ ) {
	   ...
	   if( ntbs >0 ){
		...
		printf("\n");
		segsites = gensam( list, &probss, &tmrca, &ttot, &coalTime) ;

		if(pars.cp.npop > 1){
		  int indPop = 0, iPop = 0, jPop = 0, iInd = 0;
		  int *starts = calloc(pars.cp.npop, sizeof(int));
		  int *ends = calloc(pars.cp.npop, sizeof(int));
		  char betweenPopOutputFileName[1024];
		  starts[0] = 0;		  
		  ends[0] = pars.cp.config[0];
		  
		  for( indPop = 1; indPop < pars.cp.npop; ++indPop)
		    {
		      starts[indPop]= ends[indPop-1];
		      ends[indPop] = starts[indPop] + pars.cp.config[indPop];
		    }

		  for(iPop = 0; iPop < pars.cp.npop; ++iPop){
		    double popComparison = comparePops(list, segsites, starts[iPop], ends[iPop], starts[iPop], ends[iPop]);
		    fprintf(compOut, "%f\t", popComparison);
		  }
		  
		  for(iPop = 0; iPop < pars.cp.npop-1; ++iPop){
		    for(jPop = iPop+1; jPop < pars.cp.npop; ++jPop){
		      double popComparison = comparePops(list, segsites, starts[iPop], ends[iPop], starts[jPop], ends[jPop]);
		      fprintf(compOut, "%f\t", popComparison);
		    }
		  }
		  fprintf(compOut, "\n");
		  ...
		  for(iInd = 0; iInd < pars.cp.config[0]; ++iInd)
		    {
		      double withinDifferences = indComparison(list, segsites, iInd, starts[0], ends[0]);
		      fprintf(withinCompOut, "%.8f\t", withinDifferences);
		    }
		  fprintf(withinCompOut, "\n");


		  for(iPop = 1; iPop < pars.cp.npop; ++iPop){
		    sprintf(betweenPopOutputFileName, "betweenPopOut_0_%d.txt", iPop);
		    if(count == 1)
		      betweenPopOutput = fopen(betweenPopOutputFileName, "w");
		    else if(count > 1)
		      betweenPopOutput = fopen(betweenPopOutputFileName, "a");
		    
		    for(iInd = 0; iInd < pars.cp.config[0]; ++iInd)
		      {
			double betweenDifferences = indComparison(list, segsites, iInd, starts[iPop], ends[iPop]);
			fprintf(betweenPopOutput, "%.8f\t", betweenDifferences);
		      }
		    fprintf(betweenPopOutput, "\n");
		    fclose(betweenPopOutput);
		  }

		}
		//FILE *coalTimeOut = fopen("coalout.txt", "w");
		fprintf(coalTimeOut, "%f\n", coalTime);
		//fclose(coalTimeOut);
	...
	int 
	gensam( char **list, double *pprobss, double *ptmrca, double *pttot, double *coalTime ) 
{
    ...
    if( (segsitesin == 0) && ( theta > 0.0)   ) {
	  ...
	  for( seg=0, k=0; k<nsegs; seg=seglst[seg].next, k++) { 
	    ...
	    if(seg == 0)
	      *coalTime = (seglst[0].ptree + 2*nsam -2)->time;
...
double comparePops(char** list, int ns, int i1, int i2, int j1, int j2){
  ...
  for(i =i1; i < i2; ++i)
    for(j = j1; j < j2; ++j)
      for(k = 0; k < ns; ++k)
	d += abs(list[i][k] - list[j][k]);

  return (double)d/((i2 - i1)*(j2 - j1)*(double)ns); 
}
...
double indComparison(char** list, int ns, int ix, int i1, int i2){
  ...
  for(i =i1; i < i2; ++i)
    for(j = 0; j < ns; ++j)
      d += abs(list[i][j] - list[ix][j]);

  assert(i2 - i1 > 0);

  return (double)d/( (i2 - i1) ); 
}
	...
	*phowmany = atoi( argv[2] );
	...
	while( arg < argc ){
		...
		case 'x' :
		  arg++;
		  argcheck(arg, argc, argv);
		  int st = atoi(argv[arg++]);
		  argcheck(arg, argc, argv);
		  samplingTimesPop[st-1] = atof(argv[arg++]);
		  //fprintf(stderr, "st: %d\n", st);
		  break;
			case 'p' :
				arg++;
...
usage()
{
  ...
  fprintf(stderr,"\t NEW PAVLOS Feb 2019 -x <popindex> <sampling time>  ONLY CORRECT IF POPINDEX HAS 1 INDIVIDUAL AND NO REC \n");
  fprintf(stderr,"\t NEW PAVLOS Feb 2019 -ev <popindex> <sampling time>  ONLY CORRECT IF POPINDEX HAS 1 INDIVIDUAL AND NO REC \n");  
...
make_gametes(int nsam, int mfreq, struct node *ptree, double tt, int newsites, int ns, char **list )
{
	int  tip, j,  node ;
	  ...
	  pickbmf(int nsam, int mfreq, struct node *ptree, double tt), pickb_sa(int nsam, struct node *ptree, double tt, double *samplingTimes);

	//fprintf(stderr, "samplingTimesSum: %f\n", samplingTimesSum);

	for(  j=ns; j< ns+newsites ;  j++ ) {
		if( mfreq == 1 && samplingTimesSum == 0.) node = pickb(  nsam, ptree, tt);
		else if(mfreq == 1 && samplingTimesSum > 0)
		  {
		    //fprintf(stderr, "samplingTimesSum: %f\n", samplingTimesSum);
		    node = pickb_sa(nsam, ptree, tt, samplingTimes);
		  }
...
parens( struct node *ptree, int *descl, int *descr,  int noden)
{
   ...
   if( descl[noden] == -1 ) {
	printf("%d:%5.8lf", noden+1, (ptree+ ((ptree+noden)->abv))->time );
	}
   else{
	printf("(");
...
int pickb_sa(nsam, ptree, tt, samplingTimes)
     int nsam;
     struct node *ptree;
     double tt;
     double* samplingTimes;
{
  double x, y, ran1(), samtime;
  int i;

  //fprintf(stderr, "tt_prin: %f\n", tt);
  
  double ttbefore = 0;
  for(i =0; i < 2*nsam -2; ++i)
    ttbefore += (ptree + (ptree+i)->abv )->time - (ptree+i)->time ;

  if( abs(tt - ttbefore) > 1e-4 )
    {
      fprintf(stderr, "tt: %.10lf, ttbefore: %.10lf\n", tt, ttbefore);
      //assert(tt == ttbefore);
    }
  ...
  for( i=0, y=0; i < 2*nsam-2 ; i++) {
    ...
    if(i < nsam)
      {
	samtime = samplingTimes[i];
	y += (ptree + (ptree+i)->abv )->time - samtime;
      }
    else
      y += (ptree + (ptree+i)->abv )->time - (ptree+i)->time ;
    if( y >= x )
      {
	//fprintf(stderr, "returned i: %d\n", i);
	return( i ) ;
      }
  }
