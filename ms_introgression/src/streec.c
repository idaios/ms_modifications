	...
	if( ( ( eflag == 0) && (nextevent != NULL))|| ( (nextevent != NULL) &&  ( (t+tmin) >=  nextevent->time)) ) {
	      ...
	      if( config[i] > 0)
		{
		  ...
		  for( ic = 0; ic<nchrom; ic++)
		    if( chrom[ic].pop == i ) popChromCounter++;
		  achrom = rand() % popChromCounter;
		  popChromCounter = -1;
		  for( ic = 0; ic<nchrom; ic++)
		    {
		      //fprintf(stderr, "chrom: %d, pop: %d\n", ic, chrom[ic].pop);
		      if( chrom[ic].pop == i  && (++popChromCounter) == achrom)
			{
			  //fprintf(stderr, "Line : %d migrated\n", achrom);
			  chrom[ic].pop = j;
			}
		    }
		}
	      else{
		fprintf(stderr, "Warning!!! no BACKWARD migration is possible from pop %d to pop %d. There are %d lines left in pop %d\n", i, j, config[i], i);
	      }
	      nextevent = nextevent->nextde ;		  
	      break;
